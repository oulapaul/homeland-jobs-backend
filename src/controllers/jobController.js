...
function getJobs(req, res) {
    ...
    const { search, category, location, budget_min, budget_max, sort, page = 1, limit = 10 } = req.query;
...
// POST /api/jobs
function createJobPost(req, res) {
    ...
    if (!category || !isValidCategory(category)) {
        errors.push({ field: 'category', message: `Category must be one of: ${['Frontend', 'Backend', 'Full Stack', 'Mobile', 'DevOps', 'Data Science', 'Design', 'Security', 'Management'].join(', ')}` });
    }
    
    if (!location || !isValidLocation(location)) {
        errors.push({ field: 'location', message: `Location must be one of: Nairobi, Mombasa, Kisumu, Eldoret, Remote` });
    }
...
function acceptProposal(req, res) {
    ...
    if (existingContract) {
        return res.status(409).json({ error: 'A contract already exists for this job' });
    }
...
module.exports = { getJobs, createJobPost, getJobById, submitProposal, acceptProposal };